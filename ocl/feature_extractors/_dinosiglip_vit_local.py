...
from ._base_vision_local import ImageTransform, LetterboxPad, VisionBackbone, unpack_tuple
...
class DinoSigLIPViTBackbone(VisionBackbone):
    def __init__(self, vision_backbone_id: str, image_resize_strategy: str, default_image_size: int = 224) -> None:
        ...
        if vision_backbone_id not in DINOSIGLIP_VISION_BACKBONES:
        ...
        # OpenVLA 계열과 비슷하게 두 번째 마지막 레이어 패치를 사용
        ...
        # SigLIP transform의 첫 resize를 강제로 default_image_size에 맞춤
        ...
        if self.image_resize_strategy == "resize-naive":
            ...
            assert "mean" in self.siglip_data_cfg
    ...
    def forward(self, pixel_values: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # get_intermediate_layers 반환 형식 방어
        if isinstance(dino_patches, list):
            dino_patches = dino_patches[-1]
        if isinstance(siglip_patches, list):
            siglip_patches = siglip_patches[-1]
        if isinstance(dino_patches, tuple):
            dino_patches = dino_patches[-1]
        if isinstance(siglip_patches, tuple):
            siglip_patches = siglip_patches[-1]

        if not isinstance(dino_patches, torch.Tensor):
            raise TypeError(f"dino_patches must be Tensor, got {type(dino_patches)}")
        if not isinstance(siglip_patches, torch.Tensor):
            raise TypeError(f"siglip_patches must be Tensor, got {type(siglip_patches)}")
